<template>
    <section>
      <table class="table table-hover">
        <thead>
          <tr class="bg-primary">
            <th scope="col">Room Service Type</th>
  ...
  export default {
    ...
    methods: {
      ...
      getAll() {
        this.$request.get("https://localhost:7103/v2/admin/service-room/get-all").then((res) => {
          this.roomService = res.data;
        }).catch((err) => {
    ...
    computed: {
      paginatedData() {
        ...
        return this.roomService.slice(start, end);
      },
      totalPages() {
        return Math.ceil(this.roomService.length / this.perPage);
      },
    },
    destroyed() {
